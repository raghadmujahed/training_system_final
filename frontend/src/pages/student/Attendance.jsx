...
function printForm() {
  const el = document.querySelector('.sa-form-view');
  if (!el) return;
  const win = window.open('', '_blank', 'width=900,height=700');
  win.document.write(`<!DOCTYPE html><html dir="rtl"><head><meta charset="utf-8"><title>نموذج الحضور والغياب</title><style>* { box-sizing: border-box; margin: 0; padding: 0; } body { font-family: 'Segoe UI', Tahoma, Arial, sans-serif; direction: rtl; background: #fff; } ${Array.from(document.styleSheets).flatMap(s => { try { return Array.from(s.cssRules).map(r => r.cssText); } catch { return []; } }).join('\n')}</style></head><body>${el.outerHTML}</body></html>`);
  win.document.close();
        ...
        <div className="sa-form-view">
          ...
          <div className="sa-letterhead">
            ...
            <div className="sa-lh-actions">
              ...
              <button type="button" className="sa-print-btn" onClick={printForm}>
